...
class stl_Clients extends Widget_Base {
    ...
    public function get_title() {
        return esc_html__('stl Clients', 'assurena-core');
    }
    ...
    protected function _register_controls() {
        ...
        $this->start_controls_section('stl_clients_section',
            array(
                'label'         => esc_html__('Clients Settings', 'assurena-core'),
            )
        );
        ...
        $this->add_control('item_grid',
            array(
                ...
                'options'           => [
                    ...
                    '6'          => esc_html__('Six Columns', 'assurena-core'),
                ],
                'default'           => '1',
            )
        ); 
        ...
        $repeater->add_control(
            'hover_thumbnail',
            array(
                'label'       => esc_html__('Hover Thumbnail', 'assurena-core'),
                'type'        => Controls_Manager::MEDIA,
                'label_block' => true,
                'default' => [
                    'url' => '',
                ],
                'description' => esc_html__('Need for \'Exchange Images\' and \'Shadow\' animations only.', 'assurena' ),
            )
        );
        ...
        $this->add_control('item_anim',
            array(
                'label'             => esc_html__('Thumbnail Animation', 'assurena-core'),
                'type'              => Controls_Manager::SELECT,
                'options'           => [
                    'none'          => esc_html__('None', 'assurena-core'),
                    'grayscale'          => esc_html__('Grayscale', 'assurena-core'),
                    'opacity'          => esc_html__('Opacity', 'assurena-core'),
                    'zoom'          => esc_html__('Zoom', 'assurena-core'),
                    'contrast'          => esc_html__('Contrast', 'assurena-core'),
                    'blur'          => esc_html__('Blur', 'assurena-core'),
                    'invert'          => esc_html__('Invert', 'assurena-core'),
                    'ex_images'          => esc_html__('Exchange Images', 'assurena-core'),
                    'ex_images_ver'          => esc_html__('Exchange Images Vertical', 'assurena-core'),
                    'ex_images_bg'          => esc_html__('Exchange Images with Background', 'assurena-core'),
                ],
        ...
        $this->add_control(
            'height',
            array(
                'label' => esc_html__('Custom Items Height', 'assurena-core'),
        ...
        $this->add_control(
            'item_align_v',
            array(
                'label' => esc_html__('Vertical Alignment', 'assurena-core'),
                'type' => Controls_Manager::CHOOSE,
                'options' => [
                    'flex-start'      => [
                ...
                'toggle' => true,
                'selectors' => [
                    '{{WRAPPER}} .stl-clients' => 'align-items: {{VALUE}};',
        ...
        $this->start_controls_section(
			'section_style',
			[
				'label' => esc_html__('Carousel', 'assurena-core'),
        ...
        $this->add_control(
			'image_border_radius',
			[
				'label' => esc_html__('Image Border Radius', 'assurena-core'),
    ...
    protected function render() {
        ...
        foreach ( $settings[ 'list' ] as $index => $item ) {
            ...
            if ( !empty( $item[ 'client_link' ][ 'url' ] ) ) {
                ...
                $this->add_render_attribute( $client_link, [
                    'class' => [ 'image_link','image_wrapper' ],
                    'href' => esc_url($item[ 'client_link' ][ 'url' ] ),
                    'target' => $item[ 'client_link' ][ 'is_external' ] ? '_blank' : '_self',
                    'rel' => $item[ 'client_link' ][ 'nofollow' ] ? 'nofollow' : '',
                ] );
            }
        ...
        ?><div <?php echo $this->get_render_attribute_string( 'clients' ); ?>><?php
            if((bool)$use_carousel) : echo stl_Carousel_Settings::init($carousel_options, $content, false);
            else : echo $content;
            endif;
        ?></div><?php
