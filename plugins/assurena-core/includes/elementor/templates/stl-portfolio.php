...
/**
 * stl Elementor Portfolio Template
 *
 *
 * @class        stlPortfolio
 * @version      1.0
...
class stlPortfolio
{
    ...
    public function render($atts, $self = false)
    {
        ...
        // Build Query
        list($query_args) = stl_Loop_Settings::buildQuery($atts);
        ...
        $out = '<section class="stl_cpt_section">';
        $out .= '<div class="stl-portfolio" id="' . esc_attr($item_id) . '">';
        ...
        if ($portfolio_layout == 'masonry') {
            wp_enqueue_script('isotope', stl_ELEMENTOR_ADDONS_URL . 'assets/js/isotope.pkgd.min.js');
        }
        ...
        $out .= '<div class="stl-portfolio_wrapper">';
        $out .= '<div class="stl-portfolio_container container-grid row ' . esc_attr($this->row_class($atts, $portfolio_layout)) . esc_attr($container_classes) . '" ' . $style_gap . '>';
        ...
        if ($navigation == 'pagination') {
            ...
            $out .= \assurena_Theme_Helper::pagination('10', $query_results, $nav_align);
        }
    ...
    public function output_loop_query($q, $params)
    {
        ...
        if ($q->have_posts()) :
            ...
            if ($portfolio_layout == 'masonry2' || $portfolio_layout == 'masonry3' || $portfolio_layout == 'masonry4') {
                echo '<div class="stl-portfolio-list_item-size" style="width: 25%;"></div>';
            }
            ...
            while ($q->have_posts()) : $q->the_post();
                ...
                switch ($portfolio_layout) {
                    case 'single':
                        echo $this->stl_portfolio_single_item($params, $item_class);
                        break;
                    ...
                    default:
                        ...
                        if ($navigation === 'custom_link' && $link_position === 'below_items' && $i === 1) {
                            ...
                            echo $this->stl_portfolio_item($params, $class, $i, $grid_gap, true);
                        }

                        echo $this->stl_portfolio_item($params, $item_class, $count, $grid_gap);
            ...
            $out .= $portfolio_layout == 'carousel' ? $this->stl_portfolio_carousel_item($params, $item_class, $render) : $render;
        endif;

        return $out;
    }

    public function stl_portfolio_carousel_item($params, $item_class, $return)
    {
        extract($params);
        ...
        ob_start();
        echo stl_Carousel_Settings::init($carousel_options, $return, false);
        return ob_get_clean();
    }
    ...
    private function register_css($params, $item_id)
    {
        ...
        // Gap Fix
        if ((int) $grid_gap == '0') {
            echo "#$item_id .stl-portfolio-item_image img,
                  #$item_id .inside_image .stl-portfolio-item_image {
                      border-radius: 0px;
                      width: 100%;
                  }";
        }
        ...
        // Register css
        if (!empty($styles)) {
            stl_Elementor_Helper::enqueue_css($styles);
        }
    }
    ...
    public function grid_class($params, $count, $link = false)
    {
        ...
        switch ($params['portfolio_layout']) {
            case 'masonry2':
            case 'masonry4':
                if ($count == 1 || $count == 6) $class .= 'stl_col-6';
                else $class .= 'stl_col-3';
                break;

            case 'masonry3':
                if ($count == 1 || $count == 2) $class .= 'stl_col-6';
                else $class .= 'stl_col-3';
                break;
            default:
    ...
    private function post_content($params)
    {
        ...
        $content = \assurena_Theme_Helper::modifier_character($content, $chars_count, "");

        if (!empty($content)) {
            $out .= '<div class="stl-portfolio-item_content">';
    ...
    public function stl_portfolio_item($params, $class, $count, $grid_gap, $custom_link = false)
    {
        extract($params);
        $out = $link = '';
        ...
        echo '<article class="stl-portfolio-list_item item ', esc_attr($class), '" ', $style_gap, '>';

        if ($custom_link) :
            echo $this->custom_link_item($params);

        else :
            echo '<div class="stl-portfolio-item_wrapper', esc_attr($wrapper_class), '">';
            echo $image_anim == 'offset' ? '<div class="stl-portfolio-item_offset">' : '';
            echo '<div class="stl-portfolio-item_image">';
    ...
    public function custom_link_item($params)
    {
        ...
        if (!empty($item_link['url'])) {
            $this->item->add_render_attribute('link', 'href', esc_url($item_link['url']));

            if ($item_link['is_external']) {
                $this->item->add_render_attribute('link', 'target', '_blank');
            }

            if ($item_link['nofollow']) {
                $this->item->add_render_attribute('link', 'rel', 'nofollow');
            }
        }

        $link_attributes = $this->item->get_render_attribute_string('link');
        $wrapper_class = ' align_' . $link_align;

        echo '<div class="stl-portfolio-link_wrapper', esc_attr($wrapper_class), '">',
            '<a class="stl-portfolio_item_link" ',
    ...
    public function gallery_mode_enabled($params, $link, $icon_class)
    {
        ...
        if (!empty($icon_class)) {
            echo '<div class="stl-portfolio-item_description">';
            echo '<div class="stl-portfolio-item_icon stl-portfolio-item_gallery-icon">';
            if ($info_position != 'under_image' && $image_anim != 'sub_layer') {
                echo '<i class="', esc_attr($icon_class), '"></i>';
            } else {
                echo $this->render_icon_link($params, $icon_class);
            }
    ...
    public function standard_mode_enabled($params, $link, $post_meta, $icon_class)
    {
        ...
        if ($single_link_title) {
            if ($img_click_action != 'custom' ) {
                $link_tag = 'a';
                $link_attr = ' href="' . get_permalink() . '" target="_blank"';
                $link_attr .= $img_click_action === 'popup' ? ' class="swipebox" data-elementor-open-lightbox="no"' : '';
            }
            if ($img_click_action == 'custom' && !empty(rwmb_meta('mb_portfolio_link'))) {
                $mb_custom_url = !empty(rwmb_meta('portfolio_custom_url')) ? ' href="' . rwmb_meta('portfolio_custom_url') . '"' : '';
                $url_target = !empty(rwmb_meta('portfolio_custom_url_target')) ? ' target="_blank"' : '';
    
                $link_tag = $mb_custom_url ? 'a' : 'span';
                $link_attr = $mb_custom_url . $url_target;
            }
        }
        ...
        if (!empty($icon_class)) {
            echo '<div class="stl-portfolio-item_icon">';
        ...
        echo '</div>'; // stl-portfolio-item_description
    }
    ...
    private function single_post_date()
    {
        if (rwmb_meta('mb_portfolio_single_meta_date') != 'default') {
            ...
            $date_enable = \assurena_Theme_Helper::get_option('portfolio_single_meta_date');
        }
    ...
    {
        $author = \assurena_Theme_Helper::get_option('portfolio_single_meta_author');
        if (!empty($author)) {
            return '<span class="post_author">' .
                esc_html__('by ', 'assurena-core') .
    ...
    {
        $comments_enabled = \assurena_Theme_Helper::get_option('portfolio_single_meta_comments');
        ...
        if ($comments_enabled) {
            ...
            return '<span class="comments_post">' .
                ...
                ' ' .
                esc_html(_n('Comment', 'Comments', $comments_num, 'assurena-core')) .
                '</a>' .
                '</span>';
        }
    }
    ...
    private function single_post_cats()
    {
        if (class_exists('RWMB_Loader') && rwmb_meta('mb_portfolio_single_meta_categories') != 'default') {
            ...
            $cats_enable = \assurena_Theme_Helper::get_option('portfolio_single_meta_categories');
        }
    ...
    public function stl_portfolio_single_item($parameters, $item_class = '')
    {
        ...
        if (class_exists('RWMB_Loader')) {
            ...
            $featured_image = \assurena_Theme_Helper::options_compare('portfolio_featured_image_type', 'mb_portfolio_featured_image_conditional', 'custom');
            if ($featured_image == 'replace') {
                $featured_image_replace = \assurena_Theme_Helper::options_compare('portfolio_featured_image_replace', 'mb_portfolio_featured_image_conditional', 'custom');
            }
            ...
            $mb_hide_all_meta = \assurena_Theme_Helper::get_option('portfolio_single_meta');

            if (rwmb_meta('mb_portfolio_above_content_cats') == 'default') {
                $tags_enable = \assurena_Theme_Helper::get_option('portfolio_above_content_cats');
            } else {
            ...
            if (rwmb_meta('mb_portfolio_above_content_share') != 'default') {
                ...
                $shares_enable = \assurena_Theme_Helper::get_option('portfolio_above_content_share');
            }
        }

        $single_post_align = \assurena_Theme_Helper::options_compare('portfolio_single_align', 'mb_portfolio_post_conditional', 'custom');
        ...
        // Shares
        if ($shares_enable && function_exists('stl_theme_helper')) {
            $social_share = '<div class="single_info-share_social-wpapper">' .
                $social_share .= '<div class="single_info-share_social-wpapper">';
                ob_start();
                    stl_theme_helper()->render_post_share($shares_enable);
        ...
        // Title
        $p_title = $mb_show_title ? '<h1 class="stl-portfolio-item_title">' . get_the_title() . '</h1>' : '';
        ...
        if ($mb_editor || $portfolio_info) {
            $p_annotation = '<div class="stl-portfolio__item-info">';
            if ($mb_editor) {
        ...
        // Render
        echo '<article class="stl-portfolio-single_item">';
        echo '<div class="stl-portfolio-item_wrapper">';
        echo '<div class="stl-portfolio-item_title_wrap a', $single_post_align, '">',
            $post_cats,
            $p_title,
            $p_annotation,
            $p_featured_image,
        ...
        if ($content) {
            echo '<div class="stl-portfolio-item_content">',
                '<div class="content">',
                '<div class="wrapper">',
                $content,
                '</div>',
    ...
    static public function getImgUrl(
        ...
        if (strlen($wp_get_attachment_url)) {
            if ($params['portfolio_layout'] == 'masonry2') :
                switch ($count) {
                    case '2':
                        $stl_featured_image_url = aq_resize($wp_get_attachment_url, '350', '740', $crop, true, true);
                        $masonry_gap = 'style="margin-top: -' . (33 - (int) $grid_gap) . 'px;"';
                        break;
                    default:
                        $stl_featured_image_url = aq_resize($wp_get_attachment_url, '740', '740', $crop, true, true);
                } elseif ($params['portfolio_layout'] == 'masonry3') :
                switch ($count) {
                    case '2':
                        $stl_featured_image_url = aq_resize($wp_get_attachment_url, '740', '350', $crop, true, true);
                        break;
                    default:
                        $stl_featured_image_url = aq_resize($wp_get_attachment_url, '740', '740', $crop, true, true);
                ...
                switch ($count) {
                    case 1:
                    case 6:
                        $stl_featured_image_url = aq_resize($wp_get_attachment_url, '1140', '570', $crop, true, true);
                        $masonry_gap = 'style="margin-top: -' . ((int) $grid_gap / 2) . 'px;"';
                        break;
                    default:
                        $stl_featured_image_url = aq_resize($wp_get_attachment_url, '1140', '1140', $crop, true, true);
                } else :
                if ($params['portfolio_layout'] == 'carousel' && !empty($params['variable_width'])) {
                    $stl_featured_image_url = aq_resize($wp_get_attachment_url, '1000', '600', $crop, true, true);
                } else {
                    switch ($params['posts_per_row']) { // ratio 0.7872
                        case '1':
                            $stl_featured_image_url = $wp_get_attachment_url;
                            break;
                        default:
                        case '2':
                            $stl_featured_image_url = aq_resize($wp_get_attachment_url, '1170', '1486', $crop, true, true);
                            break;
                        case '3':
                            $stl_featured_image_url = aq_resize($wp_get_attachment_url, '740', '940', $crop, true, true);
                            break;
                        case '4':
                            $stl_featured_image_url = aq_resize($wp_get_attachment_url, '570', '724', $crop, true, true);
                            break;
                        case '5':
                            $stl_featured_image_url = aq_resize($wp_get_attachment_url, '390', '495', $crop, true, true);
                            break;
                    }
                }
            ...
            if (!$stl_featured_image_url) {
                $stl_featured_image_url = $wp_get_attachment_url;
            }

            $featured_image = '<img src="' . $stl_featured_image_url . '" ' . $masonry_gap . ' alt="" />';
        }
    ...
    {
        if (null === $before) $before = __('Tags: ', 'assurena-core');
    ...
    public function getCategories($params, $query)
    {
        ...
        $out = '<a href="#" data-filter=".item" class="active">' . esc_html__('All', 'assurena-core') . '<span class="number_filter"></span></a>';
