...
class stlTeam
{
	...
	public function render($atts, $content = null)
	{
		...
		ob_start();
			$this->render_stl_team($atts);
		$team_items = ob_get_clean();

		ob_start();
		?>
		<div class="stl_module_team <?php echo esc_attr($team_classes); ?>">
			<div class="team-items_wrap">
				<?php
				switch ($use_carousel) {
					case true: 
						echo stl_Carousel_Settings::init($carousel_options, $team_items, false);
						break;
					default:
						echo \assurena_Theme_Helper::render_html($team_items);
						break;
				}
				?>
			</div>
	...
	public function render_stl_team($atts)
	{
		extract($atts);
		...
		$stl_posts = new \WP_Query($query_args);

		while ($stl_posts->have_posts()) {
			$stl_posts -> the_post();
			$counter++;
			$compile .= $this->render_stl_team_item(false, $atts, $team_image_dims, $counter);
		}
		wp_reset_postdata();
	...
	public function render_stl_team_item($single_member = false, $item_atts, $team_image_dims, $counter)
	{
		...
		if (! $hide_soc_icons && $team_icons) {
			...
			$team_icons_wrap .= '<span class="team-icon fas fa-share-alt"></span>';
		...
		if ($wp_get_attachment_url) {
			$stl_featured_image_url = ($posts_per_line == '1') ? $wp_get_attachment_url : aq_resize($wp_get_attachment_url, $team_image_dims['width'], $team_image_dims['height'], true, true, true);
			...
			$featured_image = sprintf('<%s class="team-image"><img src="%s" alt="%s" /></%s>',
				...
				esc_url($stl_featured_image_url),
		...
		if (! $single_member && ! $hide_content) {
			...
			$excerpt = \assurena_Theme_Helper::modifier_character($excerpt, $letter_count, "");
		}
		...
		if (! $single_member) :
			...
			echo '<div class="team-item">';
				echo '<div class="team-item_wrap">';
                    ...
                    echo '<div class="team-item_content">';
