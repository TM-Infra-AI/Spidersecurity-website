...
/**
* stl Elementor Loop Settings
*
*
* @class        stl_Loop_Settings
* @version      1.0
* @category     Class
* @author       StylusThemes
*/

if (! class_exists('stl_Loop_Settings')) {
	class stl_Loop_Settings
	{
		...
		{
			$query_builder = new stl_Query_Builder( $query );
		...
		public static function init($self, $atts = [])
		{
			...
			$self->start_controls_section(
				'query_section',
				[
					'label' => esc_html__( 'Query', 'assurena-core' ),
			...
			$self->add_control(
				'number_of_posts',
				[
					'label' => esc_html__( 'Post count', 'assurena-core' ),
			...
			$self->add_control(
				'order_by',
				[
					'label' => esc_html__( 'Order by', 'assurena-core' ),
					'type' => Controls_Manager::SELECT,
					'default' => 'date',
					'options' => [
						'date' => esc_html__( 'Date', 'assurena-core' ),
						'title' => esc_html__( 'Title', 'assurena-core' ),
						'author' => esc_html__( 'Author', 'assurena-core' ),
						'modified' => esc_html__( 'Modified', 'assurena-core' ),
						'rand' => esc_html__( 'Random', 'assurena-core' ),
						'comment_count' => esc_html__( 'Comments', 'assurena-core' ),
						'menu_order' => esc_html__( 'Menu Order', 'assurena-core' ),
					],
				]
			);
			...
			$self->add_control(
				'order',
				[
					'label' => esc_html__( 'Order', 'assurena-core' ),
					'type' => Controls_Manager::SELECT,
					'options' => [
						'DESC' => esc_html__( 'Descending', 'assurena-core' ),
						'ASC' => esc_html__( 'Ascending', 'assurena-core' ),
					],
					'default' => 'DESC',
				]
			);
			...
			if (! isset($atts['hide_cats'])) {
				...
				$self->add_control(
					'categories',
					[
						'label' => esc_html__( 'Filter By Category Slug', 'assurena-core' ),
				...
				$self->add_control(
					'exclude_categories',
					[
						'label' => esc_html__( 'Exclude These Categories', 'assurena-core' ),
						'type' => Controls_Manager::SWITCHER,
						'description' => esc_html__( 'Leave empty for all', 'assurena-core' ),
					]
				);  
			}           
			...
			if (! isset($atts['hide_tags']) ) :
				...
				$self->add_control(
					'tags',
					[
						'label' => esc_html__( 'Filter By Tags Slug', 'assurena-core' ),
						'type' => Controls_Manager::SELECT2,
						'multiple' => true,
						'label_block' => true,
				...
				$self->add_control(
					'exclude_tags',
					[
						'label' => esc_html__( 'Exclude These Tags', 'assurena-core' ),
						'type' => Controls_Manager::SWITCHER,
						'description' => esc_html__( 'Leave empty for all', 'assurena-core' ),
					]
				);
			...
			$self->add_control(
				'taxonomies',
				[
					'label' => esc_html__( 'Taxonomies', 'assurena-core' ),
					'type' => Controls_Manager::SELECT2, 
					'multiple' => true,
					'label_block' => true,
			...
			$self->add_control(
				'exclude_taxonomies',
				[
					'label' => esc_html__( 'Exclude These Taxonomies', 'assurena-core' ),
					'type' => Controls_Manager::SWITCHER,
					'label_on' => esc_html__( 'On', 'assurena-core' ),
					'label_off' => esc_html__( 'Off', 'assurena-core' ),
					'description' => esc_html__( 'Filter output by custom taxonomies categories, enter category names here.', 'assurena-core' ),
				]
			);
			...
			if (! isset($atts['hide_individual_posts'])) {
				...
				$self->add_control(
					'by_posts',
					[
						'label' => esc_html__( 'Individual Posts/Pages/Custom Post Types', 'assurena-core' ),
						'type' => Controls_Manager::SELECT2,
						'multiple' => true,
						'label_block' => true,
				...
				$self->add_control(
					'exclude_any',
					[
						'label' => esc_html__( 'Exclude These Posts/Pages/Custom Post Types', 'assurena-core' ),
						'type' => Controls_Manager::SWITCHER,
						'description' => esc_html__( 'Individual Posts/Pages/Custom Post Types', 'assurena-core' ),
					]
				);
			}
			...
			$self->add_control(
				'author',
				[
					'label' => esc_html__( 'Author', 'assurena-core' ),
					'type' => Controls_Manager::SELECT2,
					'multiple' => true,
					'label_block' => true,
			...
			$self->add_control(
				'exclude_author',
				[
					'label' => esc_html__( 'Exclude Author', 'assurena-core' ),
					'type' => Controls_Manager::SWITCHER,
					'label_on' => esc_html__( 'On', 'assurena-core' ),
					'label_off' => esc_html__( 'Off', 'assurena-core' ),
					'description' => esc_html__( 'Filter by author name', 'assurena-core' ),
				]
			);
		...
		public static function categories_suggester() {
			...
			foreach (get_categories() as $cat) {
				...
				$content[(string) $cat->slug] = $cat->cat_name.(! empty($parent) ? esc_html__( ' (Parent categories: (', 'assurena-core') .$parent.'))' : "");
			}
		...
		public static function taxonomies_suggester($atts)
		{
			...
			foreach ($result as $tag) {
				...
				$content[ $tag->taxonomy.":".$tag->slug ] = $tag->name . ' (' . $tag->taxonomy . ')' . (! empty($parent) ? esc_html__( ' (Parent categories: (', 'assurena-core') . $parent . '))' : '');
			}
			return $content;
		}
...
*
*
* @class       stl_Query_Builder
* @version     1.0
* @category    Class
* @author      StylusThemes
*/
if (! class_exists('stl_Query_Builder')) {
	class stl_Query_Builder
	{
		/**
		 * @since   1.0
		...
		private function get_tax($value)
		{
			...
			$taxonomies = get_terms( stl_Loop_Settings::getTaxonomies() );
			...
			$terms = get_terms( stl_Loop_Settings::getTaxonomies(),
