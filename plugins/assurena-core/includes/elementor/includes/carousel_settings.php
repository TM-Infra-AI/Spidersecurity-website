...
/**
* stl Elementor Carousel Settings
*
*
* @class        stl_Carousel_Settings
* @version      1.0
* @category     Class
* @author       StylusThemes
*/

if (! class_exists('stl_Carousel_Settings')) {
	class stl_Carousel_Settings
	{
		...
		public static function options($self, $array = [])
		{
			...
			$self->add_control(
				'use_carousel',
				[
					'label' => esc_html__('Use Carousel','assurena-core' ),
			...
			$self->add_control(
				'slides_to_scroll',
				[
					'label' => esc_html__('Slide per single item at a time','assurena-core' ),
					'type' => Controls_Manager::SWITCHER,
					'condition' => [ 'use_carousel' => 'yes' ],
					'label_on' => esc_html__( 'On', 'assurena-core' ),
					'label_off' => esc_html__( 'Off', 'assurena-core' ),
			...
			$self->add_control(
				...
				[
					...
					'selectors' => [
						...
						'{{WRAPPER}} .stl-carousel.pagination_line .slick-dots li button:before' => 'background: {{VALUE}};',
						'{{WRAPPER}} .stl-carousel.pagination_square .slick-dots li' => 'background: {{VALUE}};',
						'{{WRAPPER}} .stl-carousel.pagination_circle_border .slick-dots li.slick-active button' => 'border-color: {{VALUE}};',
						'{{WRAPPER}} .stl-carousel.pagination_circle_border .slick-dots li button:before' => 'background: {{VALUE}};',
					],
				]
			);
			...
			$self->add_control(
				'prev_next_position',
				[
					'label' => esc_html__('Arrows Positioning', 'assurena-core'),
					'type' => Controls_Manager::SELECT,
					'condition' => [ 'use_prev_next' => 'yes' ],
					'options' => [
						'' => esc_html__('Opposite each other', 'assurena-core'),
						'right' => esc_html__('Top right corner', 'assurena-core'),
					],
				]
			);
			...
			$self->add_control(
				...
				[
					'label' => esc_html__('Customize Arrows Colors','assurena-core' ),
					'type' => Controls_Manager::SWITCHER,
					'condition' => [ 'use_prev_next' => 'yes' ],
					'label_on' => esc_html__( 'On', 'assurena-core' ),
					'label_off' => esc_html__( 'Off', 'assurena-core' ),
			...
			$self->add_control(
				'prev_next_color',
				[
					'label' => esc_html__( 'Arrows Color', 'assurena-core' ),
					'type' =>  Controls_Manager::COLOR,
					'condition' => [ 'custom_prev_next_color' => 'yes' ],
					'dynamic' => [ 'active' => true ],
					'default' => '#ffffff',
			...
			$self->add_control(
				'prev_next_bg_idle',
				[
					'label' => esc_html__( 'Arrows Background Color', 'assurena-core' ),
					'type' =>  Controls_Manager::COLOR,
					'condition' => [ 'custom_prev_next_color' => 'yes' ],
                    'default' => $theme_color,
				]
			...
			$self->add_control(
				'prev_next_bg_hover',
				[
					'label' => esc_html__( 'Arrows Background', 'assurena-core' ),
					'type' =>  Controls_Manager::COLOR,
					'condition' => [ 'custom_prev_next_color' => 'yes' ],
                    'default' => '#ffffff',
				]
		...
		public static function init($atts, $item = [], $templates = false)
		{
			...
			if ($custom_pag_color || $custom_prev_next_color || ! empty($pag_offset) || ! empty($prev_next_offset)) {
				$carousel_id = uniqid( "assurena_carousel_" );
			}
			...
			// Render
			$output = '<div class="stl-carousel_wrapper">';
				$output .= '<div'.$carousel_id_attr.' class="stl-carousel'.esc_attr($carousel_wrap_classes).'">';
					$output .= '<div class="stl-carousel_slick'.$carousel_classes.'"'.$data_attribute.'>';   
						...
						if (! empty($templates)) {
							...
							$output .= ob_start();
								foreach( $item as $id ) : ?>
									<div class="item">
										<?php echo $stl_frontend->get_builder_content_for_display( $id, true );?>
									</div>
									<?php 
								endforeach; 
							$output .= ob_get_clean();
