...
class SidePanel
{
	...
	public function __construct()
	{
		...
		$this->name = __( 'Side Panel', 'assurena-core' );
		$this->plural_name = __( 'Side Panels', 'assurena-core' );
	...
	public function register()
	{
		$labels = [
			...
			'add_new'             => sprintf( __('Add New %s', 'assurena-core' ), $this->name ),
			'add_new_item'        => sprintf( __('Add New %s', 'assurena-core' ), $this->name ),
			'edit_item'           => sprintf( __('Edit %s', 'assurena-core'), $this->name ),
			'new_item'            => sprintf( __('New %s', 'assurena-core'), $this->name ),
	...
	public function wrapper_single_side_panel_open()
	{
		...
		// Get options
		$content_type = assurena_Theme_Helper::options_compare('side_panel_content_type','mb_customize_side_panel','custom');
		$pos = assurena_Theme_Helper::options_compare('side_panel_position', 'mb_customize_side_panel', 'custom');
		$side_panel_spacing = assurena_Theme_Helper::options_compare('side_panel_spacing', 'mb_customize_side_panel', 'custom');
		$icon_background = assurena_Theme_Helper::get_option('bottom_header_side_panel_background');
		$icon_color = assurena_Theme_Helper::get_option('bottom_header_side_panel_color');
	...
	{
		$bg = assurena_Theme_Helper::get_option('side_panel_bg');
		$bg = ! empty($bg['rgba']) ? $bg['rgba'] : '';

		$color = assurena_Theme_Helper::get_option('side_panel_text_color');
		$color = ! empty($color['rgba']) ? $color['rgba'] : '';

		$width = assurena_Theme_Helper::get_option('side_panel_width');
		$width = ! empty($width['width']) ? $width['width'] : '';

		$align = assurena_Theme_Helper::options_compare('side_panel_text_alignment', 'mb_customize_side_panel', 'custom');
