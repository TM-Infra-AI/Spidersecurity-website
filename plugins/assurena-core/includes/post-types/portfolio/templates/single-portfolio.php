...
$sb = assurena_Theme_Helper::render_sidebars('portfolio_single');
...
$item = new stlPortfolio();

echo '<div class="stl-portfolio-single_wrapper">';

  echo '<div class="stl-container single_portfolio', apply_filters('assurena_container_class', $container_class), '">';
	echo '<div class="row', apply_filters('assurena_row_class', $row_class), '">';
		echo '<div id="main-content" class="stl_col-', apply_filters('assurena_column_class', $column), '">';

			while ( have_posts() ) : the_post();
				echo $item->stl_portfolio_single_item($defaults, $item_class = '');
			endwhile;
			wp_reset_postdata();
			...
			// Related
			$related_switch = assurena_Theme_Helper::get_option('portfolio_related_switch');
			...
			if ($related_switch && class_exists('assurena_Core') && class_exists('Elementor\Plugin')) :
				$mb_pf_cat_r = [];

				$mb_pf_carousel_r = assurena_Theme_Helper::options_compare('pf_carousel_r', 'mb_portfolio_related_switch', 'on');
				$mb_pf_title_r = assurena_Theme_Helper::options_compare('pf_title_r', 'mb_portfolio_related_switch', 'on');
				$mb_pf_column_r = assurena_Theme_Helper::options_compare('pf_column_r', 'mb_portfolio_related_switch', 'on');
				$mb_pf_number_r = assurena_Theme_Helper::options_compare('pf_number_r', 'mb_portfolio_related_switch', 'on');
				...
				$featured_render = new stlPortfolio();
