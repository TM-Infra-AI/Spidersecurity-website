...
$sb = assurena_Theme_Helper::render_sidebars('single');
$single_type = assurena_Theme_Helper::get_option('single_type_layout');
$sb['column'] = 6;
$column = $sb['column'];
...
if ($single_type === '3') {
	echo '<div class="post_featured_bg" style="background-color: ', assurena_Theme_Helper::get_option('page_title_bg_image')['background-color'], '">';
...
<div class="stl-container<?php echo apply_filters('assurena_container_class', $container_class); ?>">
  <div class="row<?php echo apply_filters('assurena_row_class', $row_class); ?>">
	<div id='main-content' class="stl_col-<?php echo apply_filters('assurena_column_class', $column); ?>"><?php

		get_template_part('templates/services/post', $single_type);

		// Navigation
		get_template_part('templates/post/post-navigation');

		// Related Posts
		$show_post_related = assurena_Theme_Helper::get_option('single_related_posts');
		...
		if ($show_post_related && class_exists('assurena_Core') && class_exists('\Elementor\Plugin')) :

			$mb_blog_cat_r = [];

			$mb_blog_carousel_r = assurena_Theme_Helper::options_compare('blog_carousel_r', 'mb_blog_show_r', 'custom');
			$mb_blog_title_r = assurena_Theme_Helper::options_compare('blog_title_r', 'mb_blog_show_r', 'custom');

			$cats = assurena_Theme_Helper::get_option('blog_cat_r');
			if (! empty($cats)) {
			...
			$mb_blog_column_r = assurena_Theme_Helper::options_compare('blog_column_r', 'mb_blog_show_r', 'custom');
			$mb_blog_number_r = assurena_Theme_Helper::options_compare('blog_number_r', 'mb_blog_show_r', 'custom');
			...
			echo '<section class="single related_posts">';
				...
				echo '<div class="assurena_module_title">',
					'<h4>',
						! empty($mb_blog_title_r) ? esc_html($mb_blog_title_r).' ' : esc_html__('Related Posts', 'assurena').' ',
					'</h4>',
				'</div>';
				...
				$atts = [
					...
					'crop_square_img' => 1,
					'items_load' => 4,
					'name_load_more' => esc_html__('Load More', 'assurena'),
				...
				$related_items = new stlBlog();
				echo assurena_Theme_Helper::render_html($related_items->render($atts));
...
<div class="stl_col-6 single_sidebar_services">
    <?php if ( is_active_sidebar( 'services_custom_sidebar' ) ) { ?>
<?php dynamic_sidebar( 'services_custom_sidebar' ); ?>
<?php } else { echo 'Test';}?>
