...
if (! class_exists('assurena_get_header')) {
	class assurena_get_header
	{
		...
		public function header_vars() {
			...
			* @access public
			*/
			$this->header_type = assurena_Theme_Helper::get_option('header_type');

			if ($this->header_type === 'custom') {

				$header_page_select = assurena_Theme_Helper::get_option('header_page_select');

				if (! empty($header_page_select)) {
					$this->header_page_select_id = intval($header_page_select);

					if (class_exists('SitePress')) {
						$this->header_page_select_id = icl_object_id($this->header_page_select_id, 'post', false,ICL_LANGUAGE_CODE);
					}
			...
			*/
			...
			if (assurena_Theme_Helper::options_compare('header_sticky','mb_customize_header_layout','custom') == '1') {
				$header_sticky_page_select = assurena_Theme_Helper::get_option('header_sticky_page_select');
			...
			if (class_exists( 'RWMB_Loader' ) && $this->id !== 0) {
				...
				if ($customize_header == 'custom') {
					...
					if ($custom_sticky_header !== 'default') {
						...
						if (class_exists('SitePress')) {
							$this->header_sticky_page_select_id = icl_object_id( $this->header_sticky_page_select_id, 'post', false,ICL_LANGUAGE_CODE);
						}
					}
				}
			}
		...
		public function init()
		{
			...
			//hide if 404 page
			$page_not_found = assurena_Theme_Helper::get_option('404_show_header');
		...
		*/
		...
		{
			$header_shadow = assurena_Theme_Helper::get_option( 'header_shadow');
			$header_on_bg = assurena_Theme_Helper::get_option('header_on_bg');
			$header_class = '';
		...
		*/
		...
		public function header_bar_editor($location = null,$position = null)
		{
			...
			*/
			...
			${'header_'.$position.'_editor'} = assurena_Theme_Helper::get_option($location.'_header_bar_'.$position.'_editor');
		...
		*/
		...
		public function header_bar_delimiter($k = null)
		{
			...
			*/
			...
			$height = assurena_Theme_Helper::get_option( 'bottom_header_delimiter'.$get_number.'_height');
			$width = assurena_Theme_Helper::get_option( 'bottom_header_delimiter'.$get_number.'_width');

			$bg_color = assurena_Theme_Helper::get_option( 'bottom_header_delimiter'.$get_number.'_bg');

			$margin = assurena_Theme_Helper::get_option( 'bottom_header_delimiter'.$get_number.'_margin');
			...
			if ($this->html_render === 'sticky') {
				$custom_sticky = assurena_Theme_Helper::get_option('bottom_header_delimiter'.$get_number.'_sticky_custom');
				if (! empty($custom_sticky)) {
					$bg_color = assurena_Theme_Helper::get_option('bottom_header_delimiter'.$get_number.'_sticky_color');
					$height  = assurena_Theme_Helper::get_option('bottom_header_delimiter'.$get_number.'_sticky_height');
				}
			}
		...
		*/
		...
		public function header_bar_button($k = null)
		{
			...
			*/
			...
			$get_number = (int) filter_var($k, FILTER_SANITIZE_NUMBER_INT);
			$button_text = assurena_Theme_Helper::get_option('bottom_header_button'.$get_number.'_title');

			$link = assurena_Theme_Helper::get_option('bottom_header_button'.$get_number.'_link');

			$target = assurena_Theme_Helper::get_option('bottom_header_button'.$get_number.'_target');

			$size = assurena_Theme_Helper::get_option('bottom_header_button'.$get_number.'_size');

			$options_btn = $this->html_render === 'sticky' ? '_sticky' : '';

			$customize = assurena_Theme_Helper::get_option('bottom_header_button'.$get_number.'_custom'.$options_btn);

			$customize = empty($customize) ? 'def' : 'color';

			$bg_color = assurena_Theme_Helper::get_option('bottom_header_button'.$get_number.'_bg'.$options_btn);

			$bg_color = isset($bg_color['rgba']) ? $bg_color['rgba'] : '';

			$text_color = assurena_Theme_Helper::get_option('bottom_header_button'.$get_number.'_color_txt'.$options_btn);

			$text_color = isset($text_color['rgba']) ? $text_color['rgba'] : '';

			$border_color = assurena_Theme_Helper::get_option('bottom_header_button'.$get_number.'_border'.$options_btn);
			$border_color = isset($border_color['rgba']) ? $border_color['rgba'] : '';

			$bg_color_hover = assurena_Theme_Helper::get_option('bottom_header_button'.$get_number.'_hover_bg'.$options_btn);
			$bg_color_hover = isset($bg_color_hover['rgba']) ? $bg_color_hover['rgba'] : '';

			$text_color_hover = assurena_Theme_Helper::get_option('bottom_header_button'.$get_number.'_hover_color_txt'.$options_btn);
			$text_color_hover = isset($text_color_hover['rgba']) ? $text_color_hover['rgba'] : '';

			$border_color_hover = assurena_Theme_Helper::get_option('bottom_header_button'.$get_number.'_hover_border'.$options_btn);
			$border_color_hover = isset($border_color_hover['rgba']) ? $border_color_hover['rgba'] : '';

			$border_radius = assurena_Theme_Helper::get_option('bottom_header_button'.$get_number.'_radius');
			switch ($size) {
			...
			$button_css_id =  uniqid( "assurena_button_" );
			...
			// Register css
			if (! empty($styles)) {
				assurena_Theme_Helper::enqueue_css($styles);
			}
			...
			echo '<div class="header_button">';
				echo '<div class="wrapper">';
					...
					$this->add_render_attribute( 'button', 'class', 'stl-button elementor-button' );
					...
					?>
					<div <?php echo assurena_Theme_Helper::render_html($this->get_render_attribute_string( 'wrapper' )); ?>>
						<a <?php echo assurena_Theme_Helper::render_html($this->get_render_attribute_string( 'button' )); ?>>
		...
		public function render_text($settings) {
			...
			?>
			<span <?php echo assurena_Theme_Helper::render_html($this->get_render_attribute_string( 'content-wrapper' )); ?>>
				<span <?php echo assurena_Theme_Helper::render_html($this->get_render_attribute_string( 'text' )); ?>><?php echo assurena_Theme_Helper::render_html($settings['text']); ?></span>
			</span>
			<?php
		}
		...
		public function get_render_attribute_string( $element) {
			...
			return assurena_Theme_Helper::render_html_attributes( $this->render_attributes[ $element ] );
		}
		...
		*/
		...
		public function header_bar_spacer($location = null, $key = null) {
			...
			*/
			...
			$spacer = assurena_Theme_Helper::get_option($location.'_header_spacer'.$get_number);
		...
		*/
		public function build_header_layout( $section = 'bottom') {
			...
			$header_type = assurena_Theme_Helper::get_option('header_type');
		...
		public function header_default( $section = 'bottom') {
			...
			$header_layout = assurena_Theme_Helper::get_option($section.'_header_layout');
			$lavalamp_active = assurena_Theme_Helper::get_option('lavalamp_active');
					...
					if ($has_element) {
						switch ($section) {
							case 'mobile_content':
								...
								break;
							default:
								echo '<div class="stl-header-row stl-header-row-section'.esc_attr($location).'"'.$this->row_style_color($location, $section).'>';
								  echo '<div class="'.esc_attr($this->row_width_class($location, $section)).'">';
									echo '<div class="stl-header-row_wrapper"'.$this->row_style_height($location, $section).'>';
										foreach ($header_layout as $part => $value) {
											if (! empty($header_layout[$part]) && $part != 'items') {
		...
		private function row_width_class($s = '_middle', $section)
		{
			...
			*/
			...
			switch ($section) {
				case 'bottom':
					$width_container = assurena_Theme_Helper::get_option('header'.$s.'_full_width');
				...
				case 'sticky':
					$width_container = assurena_Theme_Helper::get_option('header_custom_sticky_full_width');
					if ($width_container == '1') {
						$class = "fullwidth-wrapper";
					} else {
						$class = 'stl-container';
		...
		private function row_style_color($s = '_middle', $section)
		{
			...
			switch ($this->html_render) {
				case 'bottom':
				case 'sticky':
					$header_background = assurena_Theme_Helper::get_option('header'.$s.'_background');
					$header_background_image = assurena_Theme_Helper::get_option('header'.$s.'_background_image');
					$header_background_image = isset($header_background_image['url']) ? $header_background_image['url'] : '';

					$header_color = assurena_Theme_Helper::get_option('header'.$s.'_color');
					$header_bottom_border = assurena_Theme_Helper::get_option('header'.$s.'_bottom_border');
					$header_border_height = assurena_Theme_Helper::get_option('header'.$s.'_border_height');
					$header_border_height = $header_border_height['height'];
					$header_bottom_border_color = assurena_Theme_Helper::get_option('header'.$s.'_bottom_border_color');
					...
					$customize_width = assurena_Theme_Helper::get_option('header'.$s.'_max_width_custom');

					if ($customize_width == '1') {
						$max_width = assurena_Theme_Helper::get_option( 'header'.$s.'_max_width');
		...
		private function row_style_height($s = '_middle', $section, $width = false)
		{
			...
			*/
			...
			$default_header_height = assurena_Theme_Helper::get_option('header'.$s.'_height')['height'] ?? '';
			$mobile_header_mobile = assurena_Theme_Helper::get_option('header_mobile_height')['height'] ?? '';
		...
		{
			$sticky_icon_switcher = assurena_Theme_Helper::get_option('bottom_header_side_panel_sticky_custom');
			...
			$icon_background = assurena_Theme_Helper::get_option('bottom_header_side_panel'.$value.'_background');
			$icon_color = assurena_Theme_Helper::get_option('bottom_header_side_panel'.$value.'_color');
		...
		private function column_class($s = '_middle', $section, $area)
		{
			...
			* @access private
			*/
			$dispay = assurena_Theme_Helper::get_option('header_column'.$s.'_'.$area.'_display');
			$v_align = assurena_Theme_Helper::get_option('header_column'.$s.'_'.$area.'_vert');
			$h_align = assurena_Theme_Helper::get_option('header_column'.$s.'_'.$area.'_horz');
		...
		*/
		public function build_header_mobile_menu()
		{
			...
			$header_queries = assurena_Theme_Helper::get_option('header_mobile_queris');
			...
			$sub_menu_position = assurena_Theme_Helper::get_option('mobile_position');
			...
			echo "<div class='mobile_nav_wrapper' data-mobile-width='".$header_queries."'>";
				...
				echo "<div class='container-wrapper'>";
					...
					echo "<div class='stl-menu_overlay'></div>";

					echo "<div class='stl-menu_outer", ( ! empty($sub_menu_position) ? ' sub-menu-position_'.esc_attr($sub_menu_position) : ''), "' id='stl-perfect-container'>";

						echo '<div class="stl-menu-outer_header">',
						...
						echo "<div class='stl-menu-outer_content'>";
		...
		public function header_render_html() {
			...
			echo "<header class='stl-theme-header", esc_attr($this->header_class()), "'>";

				// header output
				echo "<div class='stl-site-header", (! empty($mobile_header_custom) ? ' mobile_header_custom' : ""), "'>";
					echo "<div class='container-wrapper'>";
		...
		*/
		public static function get_logo($location, $menu = false) {
			new assurena_get_logo( $location, $menu );
		}
		/**
		* Get Header Search
		*
		...
		*/
		public function search($html_render = '',$location, $section)
		{
			...
			$customize = assurena_Theme_Helper::get_option('bottom_header_item_search_custom'.$options_btn);
			$customize = empty($customize) ? 'def' : 'color';

			$text_color = assurena_Theme_Helper::get_option('bottom_header_item_search_color_txt'.$options_btn);
			$text_color = isset($text_color['rgba']) ? $text_color['rgba'] : '';

			$text_color_hover = assurena_Theme_Helper::get_option('bottom_header_item_search_hover_color_txt'.$options_btn);
			$text_color_hover = isset($text_color_hover['rgba']) ? $text_color_hover['rgba'] : '';


			$search_css_id =  uniqid( "assurena_search_" );
		...
		public function login_in($location, $section) {
			...
			echo "<div class='login-in woocommerce'".$this->row_style_height($location, $section).">";
				...
				if (is_user_logged_in()) {
					...
					echo "<a class='login-in_link-logout' href='".esc_url( $link_logout )."'>".esc_html__('Logout', 'assurena')."</a>";
				} else {
					echo "<a class='login-in_link' href='".esc_url_raw( $url )."'>".esc_html__('Login', 'assurena')."</a>";
				}

				echo '</span>';

				echo "<div class='login-modal stl_modal-window'>";
		...
		*/
		...
		{
			echo '<div class="stl-mini-cart_wrapper">',
		...
		public function icon_cart()
		{
			...
			$customize = assurena_Theme_Helper::get_option('bottom_header_cart_custom'.$options_btn);
			$customize = empty($customize) ? 'def' : 'color';

			$text_color = assurena_Theme_Helper::get_option('bottom_header_cart_color_txt'.$options_btn);
			$text_color = isset($text_color['rgba']) ? $text_color['rgba'] : '';

			$text_color_hover = assurena_Theme_Helper::get_option('bottom_header_cart_hover_color_txt'.$options_btn);
			$text_color_hover = isset($text_color_hover['rgba']) ? $text_color_hover['rgba'] : '';

			$cart_css_id =  uniqid( "assurena_woo_" );
			...
			$this->add_render_attribute( 'cart', 'class', ['stl-cart woo_icon elementor-cart'] );
			$this->add_render_attribute( 'cart', 'role', 'button' );
			$this->add_render_attribute( 'cart', 'title', esc_attr__( 'Click to open Shopping Cart', 'assurena' ) );
			...
			echo '<a ', assurena_Theme_Helper::render_html( $this->get_render_attribute_string( 'cart' ) ), '>',
		...
		{
			ob_start();
				echo '<div class="stl-woo_mini_cart">';
