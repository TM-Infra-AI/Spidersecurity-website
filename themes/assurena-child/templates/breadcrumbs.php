...
*
*
* @class 		assurena_breadcrumbs
* @version		1.0
* @category		Class
* @author 		StylusThemes
*/

if ( ! class_exists('assurena_breadcrumbs') ) {
	class assurena_breadcrumbs {
		...
		private function text_options(){
			$this->text_home     = esc_html__( 'Home', 'assurena' ); // text for the 'Home' link
			$this->text_category = esc_html__( 'Archive by Category "%s"', 'assurena' ); // text for a category page
			$this->text_search   = esc_html__( 'Search Results for "%s" Query', 'assurena' ); // text for a search results page
			$this->text_tag      = esc_html__( 'Posts Tagged "%s"', 'assurena' ); // text for a tag page
			$this->text_author   = esc_html__( 'Articles Posted by %s', 'assurena' ); // text for an author page
			$this->text_404      = esc_html__( 'Error 404', 'assurena' ); // text for the 404 page
			$this->text_page     = esc_html__( 'Page %s', 'assurena' ); // text 'Page N'
			$this->text_cpage    = esc_html__( 'Comment Page %s', 'assurena' ); // text 'Comment Page N'		
		}
			...
			if ( is_home() || is_front_page() ) {
				...
				// custom post type
					...
					if(function_exists('assurena_woocommerce_breadcrumb')){
						...
						$args['delimiter'] = $sep;
						if ($show_home_link) $out .= $sep;
						$out .= assurena_woocommerce_breadcrumb($args);
					}
				}
				$out .= $wrap_after;

				echo assurena_Theme_Helper::render_html($out);
			}
		}
	}
	new assurena_breadcrumbs();
}