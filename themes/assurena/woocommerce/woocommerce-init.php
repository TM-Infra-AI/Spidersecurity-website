...
/**
* assurena Woocommerce
*
*
* @class        assurena_Woocoommerce
* @version      1.0
* @category     Class
* @author       StylusThemes
*/

if ( ! class_exists('assurena_Woocoommerce') ) {
	class assurena_Woocoommerce{
	    /**
		...
		*/
		...
		public function setup() {
			...
			add_theme_support( 'woocommerce', apply_filters( 'assurena_woocommerce_args', array(
				'single_image_width'    => 1080,
				'thumbnail_image_width' => 540,
				'gallery_thumbnail_image_width' => 240,
				'product_grid'          => array(
					'default_columns' => (int) assurena_Theme_Helper::get_option('shop_column'),
					'default_rows'    => 4,
					'min_columns'     => 1,
					'max_columns'     => 6,
					'min_rows'        => 1,),
		...
		public function init (){
			...
			/* stl Page Template*/
			add_action( 'woocommerce_before_main_content', array( $this, 'stl_page_template_open' ), 10 );
			add_action( 'woocommerce_after_main_content',  array( $this, 'stl_page_template_close' ), 10 );		
			/* \stl Page Template*/

			/* stl Wrapper Sorting*/
			add_action( 'woocommerce_before_shop_loop', array( $this, 'stl_sorting_wrapper_open' ), 9 );
			add_action( 'woocommerce_before_shop_loop', array( $this, 'stl_sorting_wrapper_close' ), 31 );
			...
			add_action( 'woocommerce_before_shop_loop', array( $this, 'stl_product_columns_wrapper_open' ), 40 );
			add_action( 'woocommerce_after_shop_loop', array( $this, 'stl_product_columns_wrapper_close' ), 40 );

			add_filter( 'woocommerce_product_loop_start',  array( $this, 'wrapper_catalog_shop' ) );

			add_filter( 'comment_form_fields',  array( $this, 'stl_comments_fiels' ) );
			add_filter( 'woocommerce_product_review_comment_form_args',array( $this, 'stl_filter_comments' ), 10, 1 ); 
			add_filter( 'woocommerce_product_review_list_args',array( $this, 'stl_filter_reviews' ), 10, 1 ); 
			add_filter( 'woocommerce_review_gravatar_size',array( $this, 'stl_review_gravatar_size' ), 10, 1 ); 
			
			add_filter( 'woocommerce_cart_item_thumbnail',array( $this, 'stl_image_thumbnails' ), 10, 3); 

			//Filter pagination 
			add_filter('woocommerce_pagination_args', array( $this, 'stl_filter_pagination' ) );
		}
		
		/**/
		/* stl Reviews filter */
		/**/
		function stl_filter_reviews($array){
			return array( 'callback' => array( $this, 'stl_templates_reviews' ) );
		}

		public function stl_templates_reviews($comment, $args, $depth){
		...
		public function stl_review_gravatar_size(){
			return 120;
		}
		...
		function review_comments_meta_info($comment){
			...
			if ( '0' === $comment->comment_approved ) { ?>
				<em class="woocommerce-review__awaiting-approval">
					<?php esc_html_e( 'Your review is awaiting approval', 'assurena' ); ?>
				</em>
			...
			<?php } else { ?>
				...
				if ( 'yes' === get_option( 'woocommerce_review_rating_verification_label' ) && $verified ) {
					echo '<em class="woocommerce-review__verified verified">(' . esc_attr__( 'verified owner', 'assurena' ) . ')</em> ';
				}
				?>
		...
		/**/
		/* stl Comments Form Filter */
		/**/
		function stl_filter_comments($comment_form){
			$commenter = wp_get_current_commenter();

			$comment_form = array(
				'title_reply'          => have_comments() ? esc_html__( 'Add a review', 'assurena' ) : sprintf( esc_html__( 'Be the first to review &ldquo;%s&rdquo;', 'assurena' ), get_the_title() ),
				'title_reply_to'       => esc_html__( 'Leave a Reply to %s', 'assurena' ),
				'title_reply_before'   => '<span id="reply-title" class="comment-reply-title">',
				'title_reply_after'    => '</span>',
				'comment_notes_after'  => '',
				'fields'               => array(
					'author' => '<p class="comment-form-author">' . '<label for="author"></label> ' .
					'<input id="author" name="author" placeholder="'.esc_attr__( 'Name', 'assurena' ).'" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" aria-required="true" required /></p>',
					'email'  => '<p class="comment-form-email"><label for="email"></label> ' .
					'<input id="email" name="email" placeholder="'. esc_attr__( 'Email', 'assurena' ).'" type="email" value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="30" aria-required="true" required /></p>',
				),
				'label_submit'  => esc_html__( 'Submit', 'assurena' ),
				'logged_in_as'  => '',
				'comment_field' => '',
			);
			...
			if ( $account_page_url = wc_get_page_permalink( 'myaccount' ) ) {
				...
				$comment_form['must_log_in'] = '<p class="must-log-in">' . sprintf( wp_kses( __( 'You must be <a href="%s">logged in</a> to post a review.', 'assurena' ), $allowed_html), esc_url( $account_page_url ) ) . '</p>';
			}

			if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' ) {
				$comment_form['comment_field'] = '<div class="comment-form-rating"><label for="rating">' . esc_html__( 'Your rating', 'assurena' ) . '</label><select name="rating" id="rating" aria-required="true" required>
				<option value="">' . esc_html__( 'Rate&hellip;', 'assurena' ) . '</option>
				<option value="5">' . esc_html__( 'Perfect', 'assurena' ) . '</option>
				<option value="4">' . esc_html__( 'Good', 'assurena' ) . '</option>
				<option value="3">' . esc_html__( 'Average', 'assurena' ) . '</option>
				<option value="2">' . esc_html__( 'Not that bad', 'assurena' ) . '</option>
				<option value="1">' . esc_html__( 'Very poor', 'assurena' ) . '</option>
				</select></div>';
			}

			$comment_form['comment_field'] .= '<p class="comment-form-comment"><label for="comment"></label><textarea id="comment" name="comment" cols="45" rows="8" aria-required="true" placeholder="'.esc_attr__( 'Your review', 'assurena' ).'" required></textarea></p>';
			return $comment_form;
		}
		...
		/**/
		function stl_comments_fiels( $fields ){
			if( is_product() ) {
		...
		function wrapper_catalog_shop( $ob_get_clean ){
			$animation = (bool) assurena_Theme_Helper::get_option('use_animation_shop');
			$animation_style = assurena_Theme_Helper::get_option('shop_catalog_animation_style');
			...
			echo '<ul class="stl-products'.esc_attr($classes).'">';
		}
		...
		public function loop_products_per_page() {
			return (int) assurena_Theme_Helper::get_option('shop_products_per_page');
		}
		/**/
		/* \LOOP */
		/**/
		...
		public function related_products_args( $args ) {
			$args = array(
				'posts_per_page' => (int) assurena_Theme_Helper::get_option('shop_r_products_per_page'),
		...
		public function loop_columns( $args ) {
			$columns = (int) assurena_Theme_Helper::get_option('shop_column'); // 3 products per row
			return $columns;
		}

		public function stl_product_columns_wrapper() {
			$columns = (int) assurena_Theme_Helper::get_option('shop_column');
			echo '<div class="stl-products-catalog stl-products-wrapper columns-' . absint( $columns ) . '">';
		}		
		...
		public function stl_sorting_wrapper_open(){
			echo '<div class="stl-woocommerce-sorting">';
		}		

		public function stl_sorting_wrapper_close(){
			echo '</div>';
		}

		public function stl_product_columns_wrapper_open() {
			$columns = (int) assurena_Theme_Helper::get_option('shop_column');
			echo '<div class="stl-products-catalog stl-products-wrapper columns-' . absint( $columns ) . '">';
		}		
		
		public function stl_product_columns_wrapper_close() {
			echo '</div>';
		}
		...
		public function init_template(){
			...
			$sb = assurena_Theme_Helper::render_sidebars('shop_'.$shop_template);
		...
		public function stl_page_template_open(){	    
			$this->init_template();
			?>
			<div class="stl-container single_product<?php echo esc_attr($this->container_class); ?>">
    			<div class="row<?php echo esc_attr($this->row_class); ?>">
					<div id='main-content' class="stl_col-<?php echo (int)esc_attr($this->column); ?>">
		    <?php
		}

		public function stl_page_template_close()
		{
			$this->init_template();
			echo '</div>';
		...
		public function stl_filter_pagination()
		{
		...
		public function stl_image_thumbnails( $image, $cart_item, $cart_item_key )
		{
		    $class = 'attachment-woocommerce_thumbnail size-woocommerce_thumbnail stl-woocommerce_thumbnail'; // Default cart thumbnail class.
...
new assurena_Woocoommerce ( );

// assurena Woocoommerce Helpers

if ( ! function_exists( 'assurena_woocommerce_breadcrumb' ) ) {
	/**
	 * Output the WooCommerce Breadcrumb.
	 *
	 * @param array $args Arguments.
	 */
	function assurena_woocommerce_breadcrumb( $args = array() ) {
		$args = wp_parse_args( $args, apply_filters( 'woocommerce_breadcrumb_defaults', array(
			...
			'before'      => '',
			'after'       => '',
			'home'        => esc_html_x( 'Home', 'breadcrumb', 'assurena' ),
		) ) );
		...
		if ( ! empty( $breadcrumb ) ) {
			...
			$out .= assurena_Theme_Helper::render_html($wrap_before);

			foreach ( $breadcrumb as $key => $crumb ) {

				$out .=  assurena_Theme_Helper::render_html($before);
				...
				$out .=  assurena_Theme_Helper::render_html($after);

				if ( sizeof( $breadcrumb ) !== $key + 1 ) {
					$out .=  assurena_Theme_Helper::render_html($delimiter);
				}
			}
			$out .=  assurena_Theme_Helper::render_html($wrap_after);
		}
